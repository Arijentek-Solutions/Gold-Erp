<template>
	<div class="h-full overflow-hidden">
		...
		<div class="md:hidden h-full overflow-y-auto custom-scrollbar p-3 flex flex-col gap-3">
			...
			<div class="premium-card">
				<div class="flex flex-col items-center py-4">
					<!-- Mini Clock -->
					<div
						class="w-48 h-48 rounded-full metallic-bezel border-[3px] border-[#222] flex items-center justify-center p-1 shadow-[0_0_40px_rgba(0,0,0,0.4)]"
					>
						<div
							class="w-full h-full rounded-full border-[6px] border-[#0a0c1a] flex items-center justify-center bg-[#05070a]"
						>
						...
						<button
							...
							:class="
								isOnBreak
									...
									: 'bg-white/5 border border-white/10 text-white/80 hover:bg-white/10 disabled:opacity-30'
							"
						>
			...
			<div class="grid grid-cols-2 gap-2">
				<div class="premium-card !p-3">
					...
					<div class="flex items-center gap-2">
						<div
							class="w-8 h-8 rounded-full bg-gradient-to-br from-primary/80 to-amber-600 flex items-center justify-center text-black font-bold text-xs"
						>
							{{ userInitials }}
						</div>
						<div class="min-w-0">
							...
							<p class="text-[8px] text-primary truncate">
								{{ employeeDesignation }}
							</p>
						</div>
					</div>
				...
				<div class="premium-card !p-3">
					...
					<div class="flex items-center gap-2">
						<div
							class="w-8 h-8 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400 font-bold text-[10px] uppercase"
						>
							{{ managerInitials }}
						</div>
			...
			<div class="grid grid-cols-3 gap-2">
				...
				<div class="premium-card !p-3 flex flex-col items-center">
					<p class="text-lg font-bold font-mono text-emerald-600 dark:text-emerald-400">
						{{ tasksStore.openTodos.length }}
					</p>
				...
				<div class="col-span-2">
					...
					<div class="premium-card">
						<div class="flex flex-col items-center py-2">
							<div
								class="w-56 h-56 rounded-full metallic-bezel border-[3px] border-[#222] flex items-center justify-center p-1 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
							>
								<div
									class="w-full h-full rounded-full border-[8px] border-[#0a0c1a] flex items-center justify-center bg-[#05070a]"
								>
									<div class="text-center">
										<div
										...
										<div
											class="text-[9px] text-white/30 uppercase tracking-widest mt-1"
										>
											{{ shiftLabel }}
										</div>
									</div>
					...
					<div class="grid grid-cols-2 gap-3">
						<div class="premium-card !p-4">
							...
							<div class="flex items-center gap-2">
								<div
									class="w-9 h-9 rounded-full bg-gradient-to-br from-primary/80 to-amber-600 flex items-center justify-center text-black font-bold text-xs"
								>
									{{ userInitials }}
								</div>
								<div class="min-w-0">
									...
									<p class="text-[9px] text-primary truncate">
										{{ employeeDesignation }}
									</p>
								</div>
							</div>
						...
						<div class="premium-card !p-4">
							...
							<div class="flex items-center gap-2">
								<div
									class="w-9 h-9 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400 font-bold text-[10px] uppercase"
								>
									{{ managerInitials }}
								</div>
								<p class="text-[11px] font-bold text-white truncate">
					...
					<div class="premium-card !p-4">
						<div class="flex items-center justify-between">
							<div>
								...
								<span
									v-if="payslipStatus"
									class="text-[9px] text-emerald-400 font-bold uppercase"
									>{{ payslipStatus }}</span
								>
							</div>
							<router-link to="/payroll">
								<button
									class="px-3 py-2 bg-primary/10 rounded-lg text-[9px] font-bold text-primary"
								>
									View History
								</button>
							</router-link>
		...
		<!-- Desktop Layout (xl and above) - Interactive Splitpanes -->
		<div class="hidden xl:block h-full overflow-hidden p-4">
			<splitpanes class="splitpanes-dashboard" @resize="onMainResize">
				...
				<pane
					:size="paneSizes.main[0]"
					min-size="25"
					class="pr-2 bg-transparent overflow-hidden"
				>
					<splitpanes horizontal class="splitpanes-dashboard" @resize="onLeftResize">
						<!-- Clock Section -->
						<pane
							:size="paneSizes.left[0]"
							class="pb-2 bg-transparent overflow-hidden"
						>
							<div
								class="flex-1 flex flex-col items-center justify-center py-2 premium-card relative bg-gradient-to-b from-white/5 to-transparent h-full"
							>
								<div
									class="absolute top-0 left-0 w-full h-full diamond-pattern opacity-10 pointer-events-none overflow-hidden"
								></div>
								<div
									class="relative group z-10 scale-[0.8] lg:scale-90 flex-1 flex flex-col justify-center w-full items-center"
								>
									<div
										class="w-[300px] h-[300px] lg:w-[340px] lg:h-[340px] rounded-full metallic-bezel border-[4px] border-[#222] flex items-center justify-center relative p-1 shadow-[0_0_80px_rgba(0,0,0,0.6)]"
									>
										<div
										...
										>
											...
											>
												<div
													class="absolute top-[22%] text-[8px] text-white/20 tracking-[0.5em] font-bold uppercase font-display"
												>
													{{ shiftLabel }}
												</div>
												<div
													class="text-[3rem] lg:text-[4rem] leading-none font-mono font-bold tracking-tighter mt-2 transition-colors duration-500 z-10 drop-shadow-2xl tabular-nums"
													:class="getTimerColor"
												>
													{{ displayTime }}
												</div>
												<div
													class="absolute bottom-[22%] flex gap-2"
													v-if="attendance.isCheckedIn"
												>
													<span
														class="w-1.5 h-1.5 rounded-full animate-pulse bg-primary"
													></span>
													<span
														class="w-1.5 h-1.5 rounded-full opacity-20 bg-primary"
													></span>
												</div>
											</div>
										</div>
						...
						<!-- People Section -->
						<pane
							:size="paneSizes.left[1]"
							class="pt-2 bg-transparent overflow-hidden"
						>
							<div class="grid grid-cols-2 gap-2 h-full">
								<div class="premium-card !p-4 flex flex-col justify-between">
									...
									<div class="flex items-center gap-3">
										<div
											class="w-8 h-8 rounded-full bg-gradient-to-br from-primary/80 to-amber-600 flex items-center justify-center text-black font-bold text-[10px] shadow-lg"
										>
											{{ userInitials }}
										</div>
										<p
											class="text-[10px] font-bold text-gray-900 dark:text-white truncate"
										>
											{{ employeeName }}
										</p>
									</div>
								...
								<div class="premium-card !p-4 flex flex-col justify-between">
									...
									<div class="flex items-center gap-3">
										<div
											class="w-8 h-8 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400 font-bold text-[10px] uppercase border border-blue-500/20"
										>
											{{ managerInitials }}
										</div>
										<p
				...
				<pane
					:size="paneSizes.main[1]"
					min-size="30"
					class="px-2 bg-transparent overflow-hidden"
				>
					<splitpanes horizontal class="splitpanes-dashboard" @resize="onCenterResize">
						<!-- Upcoming Roster -->
						<pane
							:size="paneSizes.center[0]"
							class="pb-2 bg-transparent overflow-hidden"
						>
							<div class="premium-card !p-7 h-full flex flex-col">
								<div class="flex items-center justify-between mb-6">
									<div>
										...
										<p
											class="premium-subtitle !text-[9px] uppercase tracking-widest mt-1"
										>
											Next 5 Shifts
										</p>
									</div>
									...
									>
										<span
											class="material-symbols-outlined text-gray-400 dark:text-white/30 text-base"
											>calendar_view_week</span
										>
									</div>
								</div>
								<div
									class="space-y-2 flex-1 overflow-y-auto custom-scrollbar pr-2"
								>
									<div
										v-for="roster in upcomingRoster"
										:key="roster.date"
										class="bg-white/5 p-3 rounded-xl border border-white/5 flex items-center justify-between group hover:bg-white/[0.03] transition-all"
									>
										<div class="flex items-center gap-4">
											<div class="text-center min-w-[36px]">
												<p
													class="text-[9px] uppercase font-black text-white/30"
												>
													{{ roster.dayShort }}
												</p>
												<p
											...
											<div>
												...
												<p class="text-[9px] text-white/30 font-mono">
													{{ roster.shift_time }}
												</p>
											</div>
										</div>
										<div class="text-right">
											<span
												class="text-[8px] font-black px-2 py-0.5 rounded bg-primary/10 text-primary uppercase tracking-widest"
											>
												{{ roster.hours }}H
											</span>
										</div>
							...
							<div class="premium-card !p-6 h-full flex flex-col justify-between">
								<div class="flex items-center justify-between">
									<div>
										...
										>
											<span
												class="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.4)]"
											></span>
											<span
												class="text-[9px] text-emerald-400 font-bold uppercase tracking-wider"
												>{{ payslipStatus }}</span
											>
										</div>
									</div>
									<router-link to="/payroll">
										...
										>
											<span
												class="material-symbols-outlined text-gray-400 dark:text-white/40 text-lg"
												>receipt_long</span
											>
										</button>
									</router-link>
								...
								<button
									class="mt-4 py-2.5 bg-primary/5 hover:bg-primary/10 rounded-xl text-[10px] font-bold text-primary border border-primary/10 transition-all"
								>
									Download Statement
								</button>
							</div>
				...
				<pane
					:size="paneSizes.main[2]"
					min-size="20"
					class="pl-2 bg-transparent overflow-hidden"
				>
					<splitpanes horizontal class="splitpanes-dashboard" @resize="onRightResize">
						<!-- Activity -->
						<pane
							:size="paneSizes.right[0]"
							class="pb-2 bg-transparent overflow-hidden"
						>
							<div class="premium-card !p-7 h-full flex flex-col">
								<div class="flex items-center justify-between mb-6">
								...
								>
									...
									>
										<div
											class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border bg-black/20 border-white/5 transition-all group-hover:border-primary/30"
										...
										<div class="min-w-0">
											<p
												class="text-[10px] font-bold text-gray-900 dark:text-white/90 group-hover:text-primary transition-colors truncate"
											>
												{{ log.title }}
											</p>
											<p class="text-[8px] text-white/30 font-mono">
												{{ log.time }}
											</p>
										</div>
									</div>
						...
						>
							<div class="premium-card !p-7 h-full flex flex-col">
								<div class="flex items-center justify-between mb-6">
									...
									<button
										@click="showAddTodo = true"
										class="w-8 h-8 rounded-xl bg-primary text-black hover:bg-yellow-400 flex items-center justify-center transition-all shadow-lg shadow-primary/20"
									>
								...
								<div
									class="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2"
								>
									<div
										v-for="todo in sortedOpenTodos"
										:key="todo.id"
										@click="toggleTodo(todo.id, todo.status)"
										class="group flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] transition-all border border-white/5 cursor-pointer"
									>
										<div
											class="w-4 h-4 rounded border border-white/10 group-hover:border-primary transition-all"
										></div>
										<p
											class="text-[10px] font-bold text-gray-900 dark:text-white group-hover:text-primary/90 transition-colors truncate"
										>
											{{ todo.description }}
										</p>
									</div>
		...
		<Teleport to="body">
			...
			>
				...
				>
					...
					<div class="mb-4">
						<label
							class="text-[10px] text-white/40 uppercase tracking-widest mb-2 block"
							>Priority</label
						>
...
const isOnBreak = computed(() => attendance.isOnBreak);
...
const getTimerColor = computed(() => {
	if (!attendance.isCheckedIn && !attendance.isOnBreak && !attendance.workedSecondsToday) {
		return "text-white/20";
	}
	if (attendance.shiftComplete) return "text-green-500";
	if (attendance.isOnBreak) return "text-sky-400";
	return "text-amber-500";
});

const getDotColor = computed(() => {
	if (attendance.shiftComplete) return "bg-green-500";
	if (attendance.isOnBreak) return "bg-sky-400";
	return "bg-amber-500";
});
...
const upcomingRoster = computed(() => {
	...
	for (let i = 0; i < 5; i++) {
		...
		days.push({
			...
			shift_time: `${attendance.roster?.start_time?.substring(0, 5) || "09:00"} - ${
				attendance.roster?.end_time?.substring(0, 5) || "17:00"
			}`,
...
const recentActivity = computed(() => {
	...
	return attendance.todayStatus.logs
		.map((log, index) => {
			const isBreakStart = log.note === "Break Start";
			const isBreakEnd = log.note === "Break End";

<template>
	<Transition name="fade">
		<div class="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6">
			...
			<div class="relative bg-white dark:bg-[#1a1c23] rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-transparent dark:border-white/10">
				<!-- Header -->
				<div class="flex items-center justify-between p-6 border-b border-gray-100 dark:border-white/5">
					<div>
						<h2 class="text-lg font-bold text-gray-900 dark:text-white">Process Payment</h2>
						<p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Layaway: {{ layawayId }}</p>
					</div>
				...
				<div class="p-6 space-y-4">
					<!-- Balance Info -->
					<div class="bg-gradient-to-br from-orange-50 to-orange-100/50 dark:from-orange-900/20 dark:to-orange-800/10 rounded-xl p-4 border border-orange-200 dark:border-orange-800/30">
					...
					<!-- Mode of Payment -->
					<div>
						<label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Mode of Payment</label>
					...
					<!-- Payment Summary -->
					<div v-if="paymentAmount > 0" class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-700/50">
						<div class="space-y-2">
							<div class="flex justify-between text-sm">
								<span class="text-gray-500 dark:text-gray-400">Payment Amount</span>
								<span class="text-gray-900 dark:text-white font-bold">{{ formatCurrency(paymentAmount) }}</span>
							</div>
							<div class="flex justify-between text-sm">
								<span class="text-gray-500 dark:text-gray-400">Remaining Balance</span>
								<span class="text-green-600 dark:text-green-400 font-bold">{{ formatCurrency(balanceAmount - paymentAmount) }}</span>
							</div>
							<div v-if="balanceAmount - paymentAmount <= 0" class="flex justify-between text-sm pt-2 border-t border-gray-200 dark:border-gray-700">
								<span class="text-blue-600 dark:text-blue-400 font-medium">Contract Status</span>
								<span class="text-blue-600 dark:text-blue-400 font-bold">Will be Completed</span>
							</div>
						</div>
					</div>

					<!-- Error Message -->
					<div v-if="error" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 rounded-xl p-3">

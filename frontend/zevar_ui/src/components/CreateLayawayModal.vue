<template>
	<Transition name="fade">
		<div v-if="show" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
			...
			<div
				class="relative bg-white dark:bg-[#1a1c23] rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden border border-transparent dark:border-white/10"
			>
				<!-- Header -->
				<div
				...
				<div class="p-6 overflow-y-auto" style="max-height: calc(90vh - 200px)">
					...
					<div v-if="currentStep === 1">
						...
						<div v-if="customerResults.length > 0" class="space-y-2 mb-4">
							...
							>
								...
								<p class="text-sm text-gray-500">
									{{ customer.mobile_no || customer.email_id || 'No contact' }}
								</p>
							</button>
						</div>
						...
						>
							...
							<p class="text-sm text-green-600 dark:text-green-400">
								{{ selectedCustomer.mobile_no || 'No phone' }}
							</p>
						</div>
					</div>
					...
					<div v-if="currentStep === 2">
						...
						<div
							...
							class="grid grid-cols-2 gap-2 mb-4 max-h-40 overflow-y-auto"
						>
							<button
						...
						<div class="space-y-4">
							...
							<div>
								...
								<p class="text-xs text-gray-500 mt-1">
									Minimum deposit: ${{ formatPrice(minDeposit) }}
								</p>
							</div>
							...
							<div
								v-if="form.deposit && form.duration"
							...
							>
								...
								<p class="text-xs text-gray-500 mt-1">
									Remaining balance: ${{ formatPrice(balanceAmount) }} over
					...
					<div v-if="currentStep === 5">
						...
						>
							<p class="text-xs text-yellow-700 dark:text-yellow-400">
								<strong>Note:</strong> You can skip payment now and collect deposit
								later. The layaway will be created with "Pending" status.
							</p>
						</div>
					</div>
						...
						<div class="space-y-4">
							...
							<label class="flex items-start gap-3 cursor-pointer">
								...
								<span class="text-sm text-gray-600 dark:text-gray-400">
									I confirm that the customer has agreed to the layaway terms
				...
				<!-- Footer -->
				<div
					class="flex items-center justify-between p-6 border-t border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-gray-900/30"
				>
					<button
						v-if="currentStep > 1"
						@click="prevStep"
...
const paymentSchedule = computed(() => {
	...
	for (let i = 1; i <= form.value.duration; i++) {
		...
		schedule.push({
			...
			amount:
				i === form.value.duration
					? balanceAmount.value - monthlyPayment.value * (form.value.duration - 1)

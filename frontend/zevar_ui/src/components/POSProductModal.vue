    ...
    <div v-if="show" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div class="relative bg-white dark:bg-[#1a1c23] rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] border border-transparent dark:border-white/10 transition-all">
        ...
        <button @click="close" class="absolute top-4 right-4 z-10 p-2 bg-white/80 dark:bg-black/50 backdrop-blur rounded-full hover:bg-gray-100 dark:hover:bg-white/20 transition-colors group">
            <svg class="h-6 w-6 text-gray-600 dark:text-white group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="w-full md:w-1/2 p-8 overflow-y-auto bg-white dark:bg-[#1a1c23]">
            ...
            <div v-else>
                ...
                <div class="flex gap-2 mb-8">
                    <span v-if="details.metal" class="px-3 py-1 bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-500 rounded text-xs font-bold uppercase tracking-wider border border-yellow-200 dark:border-yellow-700/50">
                        {{ details.metal }}
                    </span>
                    <span v-if="details.purity" class="px-3 py-1 bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300 rounded text-xs font-bold uppercase tracking-wider border border-gray-200 dark:border-gray-700">
                        {{ details.purity }}
                    </span>
                </div>

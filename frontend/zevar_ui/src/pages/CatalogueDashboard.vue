...
<div class="min-h-screen" :class="isDark ? 'bg-[#0a0a0a]' : 'bg-white'">
  ...
  <section class="relative overflow-hidden bg-[#faf5f0]" style="height: 520px">
    <TransitionGroup name="hero-slide" tag="div" class="relative w-full h-full">
      <div v-for="(slide, i) in heroSlides" :key="slide.id" v-show="currentSlide === i" class="absolute inset-0">
        ...
        <div class="absolute inset-0" :style="{ background: slide.overlay }">
          <div class="max-w-7xl mx-auto px-6 h-full flex items-center" :class="slide.align === 'center' ? 'justify-center text-center' : ''">
            <div class="max-w-lg" :class="slide.align === 'center' ? '' : ''">
              <p v-if="slide.subtitle" class="text-[#8B6914] text-sm tracking-[0.25em] uppercase mb-3 font-semibold">{{ slide.subtitle }}</p>
              <h2 class="text-4xl md:text-5xl font-serif font-bold text-gray-900 mb-4 leading-tight" v-html="slide.title"></h2>
              <p v-if="slide.description" class="text-gray-700 text-lg mb-6 font-light">{{ slide.description }}</p>
              <button @click="scrollTo('shop-categories')" class="px-8 py-3.5 bg-[#8B6914] text-white font-bold tracking-wider uppercase text-sm rounded hover:bg-[#7a5c11] transition-all shadow-lg">
    ...
    <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">
      <button v-for="(_, i) in heroSlides" :key="i" @click="currentSlide = i" class="w-2.5 h-2.5 rounded-full transition-all" :class="currentSlide === i ? 'bg-[#8B6914] w-6' : 'bg-gray-400/50 hover:bg-gray-400'"></button>
    </div>
    <!-- Arrows -->
    <button @click="prevSlide" class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white transition z-10">
    ...
    <button @click="nextSlide" class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/80 rounded-full flex items-center justify-center shadow hover:bg-white transition z-10">
  ...
  <section class="py-16 bg-white">
    <div class="max-w-6xl mx-auto px-6">
      <div class="text-center mb-10">
        <h2 class="text-3xl font-serif font-bold text-gray-900">Signature Diamond Collection</h2>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
        <div v-for="col in signatureCollections" :key="col.name" class="group cursor-pointer" @click="handleCategorySelect(col.link)">
          <div class="relative overflow-hidden rounded-2xl aspect-[3/4]">
            ...
            <p class="absolute bottom-5 left-0 right-0 text-center text-white font-semibold text-sm tracking-wide">{{ col.name }}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <section id="shop-categories" class="py-16 bg-white">
    <div class="max-w-6xl mx-auto px-6">
      <div class="text-center mb-10">
        <h2 class="text-3xl font-serif font-bold text-gray-900">Find Your Perfect Match</h2>
        <p class="text-[#8B6914] mt-2 font-semibold tracking-wide">Shop by Categories</p>
      </div>
      <div class="grid grid-cols-4 gap-x-6 gap-y-8 max-w-3xl mx-auto">
        <div v-for="cat in shopCategories" :key="cat.name" class="flex flex-col items-center group cursor-pointer" @click="handleCategorySelect(cat.link)">
          <div class="w-28 h-28 md:w-36 md:h-36 rounded-2xl overflow-hidden border-2 border-transparent group-hover:border-[#C9A962] transition-all duration-300 shadow-sm group-hover:shadow-lg">
            <img :src="cat.image" :alt="cat.name" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
          </div>
          <p class="mt-3 text-sm font-bold text-gray-700 uppercase tracking-wider group-hover:text-[#8B6914] transition-colors">{{ cat.name }}</p>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="py-16 bg-white">
    <div class="max-w-6xl mx-auto px-6">
      <div class="text-center mb-10">
        ...
        <p class="text-gray-600 mt-2 font-light">Jewelry pieces everyone's eyeing right now</p>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-for="item in trendingItems" :key="item.name" class="group cursor-pointer" @click="openProduct(item)">
          ...
          <p class="text-center mt-4 text-gray-800 font-medium tracking-wide">{{ item.name }}</p>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="py-16 bg-[#faf5f0]">
    <div class="max-w-6xl mx-auto px-6">
      ...
      <div class="grid grid-cols-2 gap-5 max-w-4xl mx-auto">
        <div v-for="world in zevarWorld" :key="world.name" class="relative overflow-hidden rounded-2xl aspect-[4/3] cursor-pointer group" @click="handleCategorySelect(world.link)">
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end justify-center pb-6">
            <p class="text-white font-serif text-2xl font-light tracking-wide italic">{{ world.name }}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="py-16 bg-[#e8ddd0] relative overflow-hidden">
    <img src="https://images.unsplash.com/photo-1601121141461-9d6647bca1ed?w=1920&q=60" class="absolute inset-0 w-full h-full object-cover opacity-30" />
    <div class="relative max-w-6xl mx-auto px-6">
      <div class="mb-8">
        <h2 class="text-3xl font-serif font-bold text-gray-900">New Arrivals</h2>
        <span class="inline-block ml-3 px-3 py-1 bg-[#8B6914] text-white text-xs font-bold rounded-full">100+ New Items</span>
        <p class="text-gray-700 mt-2 font-light">Fresh designs dropping daily. Explore the latest launches.</p>
      </div>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
    ...
    <div class="max-w-5xl mx-auto px-6">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
        <div v-for="badge in trustBadges" :key="badge.title" class="flex flex-col items-center text-center gap-3">
          <div class="w-16 h-16 rounded-full bg-[#faf5f0] flex items-center justify-center">
            <svg class="w-7 h-7 text-[#8B6914]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :d="badge.icon"/></svg>
          </div>
          <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-for="g in genderShop" :key="g.name" class="group cursor-pointer" @click="handleCategorySelect(g.link)">
          <div class="relative overflow-hidden rounded-2xl aspect-[3/4]">
            ...
            <div class="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-end justify-center pb-6">
              <p class="text-white font-serif text-2xl font-light tracking-wide">{{ g.name }}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <section class="py-16 bg-[#faf5f0]">
    <div class="max-w-6xl mx-auto px-6">
      <div class="text-center mb-10">
        <h2 class="text-3xl font-serif font-bold text-gray-900">The Zevar Experience</h2>
        <p class="text-gray-600 mt-2 font-light">Find a Boutique or Book a Consultation</p>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-for="exp in experiences" :key="exp.title" class="group cursor-pointer">
          ...
          <p class="text-center mt-4 text-gray-800 font-bold uppercase tracking-wider text-sm">{{ exp.title }}</p>
        </div>
      </div>
    </div>
  </section>
        ...
        <div v-if="showPartners">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
            <div v-for="partner in partnerBrands" :key="partner.name" 
              ...
              <p class="text-xs font-light text-center" :class="isDark ? 'text-gray-500' : 'text-gray-400'">{{ partner.desc }}</p>
            </div>
          </div>
    ...
    <div class="max-w-6xl mx-auto px-6">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
        ...
        <div>
          ...
          <p class="text-gray-400 text-sm mb-4 font-light">hello@zevarjewelers.com</p>
      ...
      <div class="border-t border-white/10 pt-6 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-gray-500 font-light">
        <p>&copy; {{ new Date().getFullYear() }} ZEVAR Jewelers. All rights reserved.</p>
...
// ---- ZEVAR EXPERIENCE ----
const experiences = [
  { title: 'Visit Our Store', image: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=600&q=80' },
  { title: 'Book an Appointment', image: 'https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=600&q=80' },
  { title: 'Talk to an Expert', image: 'https://images.unsplash.com/photo-1573408301185-9146fe634ad0?w=600&q=80' }
]
...
const partnerBrands = [
  ...
  { name: 'Stuller', initial: 'S', desc: 'Fine Jewelry & Diamonds', url: 'https://www.stuller.com' },
...
.partner-fade-enter-active, .partner-fade-leave-active {

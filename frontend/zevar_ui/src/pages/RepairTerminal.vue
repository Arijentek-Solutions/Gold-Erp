		...
		<div class="h-full flex flex-col">
			...
			<div v-if="stats" class="flex gap-4 mb-6 text-sm">
				<span class="text-gray-500 dark:text-gray-400"
					>Received:
					<b class="text-gray-900 dark:text-white">{{ stats.Received || 0 }}</b></span
				>
				<span class="text-gray-500 dark:text-gray-400"
					>In Progress:
				...
				>
					...
					<form @submit.prevent="submitNewRepair" class="space-y-4">
						<div>
							...
							/>
							<p v-if="!newForm.customer" class="text-xs text-red-500 mt-1">
						...
						<div>
							...
							>
								...
								<option v-for="t in repairTypes" :key="t.name" :value="t.name">
									{{ t.repair_name }} ({{
						...
						<div class="flex gap-2 pt-4">
							<button
								type="submit"
								class="flex-1 py-2 bg-[#8B6914] text-white rounded-lg font-bold"
							>
								Create
							</button>
							<button
...
async function updateStatus(name) {
	try {
		...
		toast({
			title: 'Error',
			message: e.messages?.[0] || 'Failed to update status',
			icon: 'alert-triangle',
			intent: 'error',
		})
	}

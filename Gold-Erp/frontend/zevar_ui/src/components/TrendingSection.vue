<template>
	<section class="px-4 sm:px-6 py-3 overflow-x-hidden">
		...
		<div class="flex items-center justify-between mb-3">
			<div class="flex items-center gap-2">
				<div
					...
					style="background: linear-gradient(135deg, #c9a962 0%, #8b7355 100%)"
				>
					<svg
			...
			<button
				...
				class="w-6 h-6 rounded flex items-center justify-center transition-all hover:scale-105"
				:class="[
		...
		<div v-if="isLoading" class="flex justify-center py-6">
			<div
				class="w-5 h-5 border-2 border-[#C9A962] border-t-transparent rounded-full animate-spin"
			></div>
		</div>
		...
		<div
			...
			class="text-center py-6 rounded-lg border"
			:class="isDark ? 'bg-[#12121a] border-white/5' : 'bg-gray-50 border-gray-200'"
		>
				...
				>
					...
					>
						...
						<div
							...
							:class="isDark ? 'bg-[#0a0a0c]' : 'bg-gray-50'"
						>
							<!-- Rank -->
							<div
								class="absolute top-1 left-1 z-10 w-4 h-4 rounded flex items-center justify-center text-[9px] font-bold"
								:class="
									isDark
										? 'bg-black/60 text-[#C9A962]'
							...
							<div
								v-if="item.is_hot"
								class="absolute top-1.5 right-1.5 z-10 px-1.5 py-0.5 rounded-md text-[8px] font-bold text-white shadow-sm"
								style="
						...
						<!-- Content -->
						<div class="p-2">
							<p class="text-[8px] font-medium text-[#C9A962]">
								{{ item.category }}
							</p>
							<h3
...
const displayItems = computed(() => {
	...
	if (props.category && props.category !== 'all')
		return items.filter((item) => item.categoryId === props.category)
	return items
})

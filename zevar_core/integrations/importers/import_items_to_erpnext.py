				...
				doc = frappe.get_doc(
					{
						...
						"is_group": 1
						if g["name"]
						in ["Jewelry", "Watches", "Necklaces & Chains", "Rings", "Earrings", "Bracelets"]
						else 0,
					}
				)
...
def map_demo_item(record):
	...
	return {
		...
		"item_name": name[:140],
		"item_group": category
		if category in ["Rings", "Chains", "Earrings", "Bracelets", "Pendants", "Watches"]
		else "Products",
		"stock_uom": "Nos",
		"is_stock_item": 1,
		"include_item_in_manufacturing": 0,
		...
		"custom_source": "Demo",
		"custom_gender": random.choice(["Unisex", "Men's", "Women's"]),
		"custom_length_value": round(random.uniform(6, 24), 1)
		if category in ["Chains", "Necklaces", "Bracelets"]
		else None,
		...
		"custom_clasp_type": random.choice(CLASP_TYPES)
		if category in ["Chains", "Necklaces", "Bracelets"]
		else None,

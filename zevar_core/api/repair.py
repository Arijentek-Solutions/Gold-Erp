...
def get_repair_orders(
    ...
    orders = frappe.get_list(
        ...
        fields=[
            ...
            "handled_by", "repair_type", "item_description", "estimated_cost",
            "total_cost", "payment_status", "received_date", "promised_date",
            "completed_date", "delivered_date", "assigned_to", "warehouse"
        ],
...
def get_repair_stats(warehouse: Optional[str] = None) -> Dict[str, int]:
    ...
    statuses = ["Received", "Estimated", "Approved", "In Progress", "Quality Check", "Ready for Pickup", "Delivered", "Cancelled"]
    counts = {}
    for s in statuses:
...
def update_repair_status(name: str, status: str, work_notes: Optional[str] = None) -> Dict[str, str]:
    ...
    if status in ("In Progress", "Quality Check", "Ready for Pickup") and not doc.completed_date and status == "Ready for Pickup":
        from frappe.utils import now
...
def get_repair_order_details(name: str) -> Dict[str, Any]:
    ...
    if doc_dict.get("repair_type"):
        doc_dict["repair_type_name"] = frappe.db.get_value("Repair Type", doc_dict["repair_type"], "repair_name")
...
def get_customer_repair_history(customer: str, limit: int = 20) -> List[Dict[str, Any]]:
    ...
    orders = frappe.get_all(
        ...
        fields=["name", "status", "repair_type", "item_description", "total_cost", "received_date", "delivered_date"],

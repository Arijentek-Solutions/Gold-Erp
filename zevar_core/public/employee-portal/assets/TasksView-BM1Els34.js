...
const h = { class: "h-full flex flex-col gap-6 p-10" },
	u = { class: "flex-1 overflow-y-auto custom-scrollbar" },
	m = { class: "grid grid-cols-1 md:grid-cols-3 gap-6 pb-4 h-full" },
	b = {
		class: "flex items-center gap-3 mb-4 p-2 bg-white/5 rounded-lg border border-white/5 shrink-0",
	},
	x = { class: "text-xs font-black uppercase tracking-widest text-white/70" },
	f = {
		class: "ml-auto text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/50 font-mono",
	},
	y = { class: "space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-2" },
	w = { class: "flex items-start justify-between mb-3" },
	_ = { class: "text-[10px] text-white/30" },
	v = {
		class: "font-bold text-sm mb-2 text-white group-hover:text-primary transition-colors leading-snug",
	},
	C = { class: "text-xs text-white/40 leading-relaxed mb-4" },
	...
	S = { class: "text-[9px] font-bold text-white/70" },
	j = { class: "flex-1 h-1 bg-white/5 rounded-full overflow-hidden" },
	M = {
		__name: "TasksView",
		setup(A) {
			const c = [
			...
			return (z, i) => (
				...
				r("div", h, [
					i[0] ||
						(i[0] = g(
							'<div class="flex items-center justify-between shrink-0"><div><h2 class="text-3xl font-bold font-display text-white mb-1">My Tasks</h2><p class="text-white/50 text-sm">Track and manage your daily jewelry collection goals</p></div></div><div class="shrink-0"><div class="glass-card p-4 rounded-xl flex items-center gap-4"><div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary"><span class="material-symbols-outlined">add_task</span></div><input type="text" placeholder="Add a new task to your board..." class="flex-1 bg-transparent border-none outline-none text-white placeholder:text-white/20 text-sm"><button class="bg-primary hover:bg-yellow-400 text-black px-6 py-2 rounded-lg text-xs font-bold tracking-wide transition-all uppercase"> Add Task </button></div></div>',
							2
						)),
					e("div", u, [
						e("div", m, [
							...
							r(
								...
								null,
								d(c, (a) =>
									e("div", { key: a.title, class: "flex flex-col h-full" }, [
										e("div", b, [
											e(
												"div",
												{
